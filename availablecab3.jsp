                ...
                %>
                    ...
                    %><tr><% while(rs.next())
                    {
                                ...
                                <td><%=rs.getString("contactnumber")%></td>
                                 ...
                                 <td><a href="confirmbooking.jsp?dname=<%=rs.getString("drivername")%>&pinc=<%=rs.getString("pincode")%>&tot=<%=tot%>&vnum=<%=rs.getString("vehiclenumber")%>&vname=<%=rs.getString("vehicletype")%>&connum=<%=rs.getString("contactnumber")%>&scap=<%=rs.getString("seatcapacity")%>&km=<%=rs.getString("rateperkm")%>&pcity=<%=request.getParameter("pcity")%>&dcity=<%=request.getParameter("dcity")%>&tdate=<%=request.getParameter("tdate")%>&ptime=<%=request.getParameter("ptime")%>&sc=<%=request.getParameter("sc")%>&day=<%=request.getParameter("day")%>"> <input type="submit" value="select"></a></td></tr>
